...
matchesThreshold = 0.8
# how many good matches are required to "find" the object
...
out = cv2.VideoWriter('output.mp4', fourcc, 30.0, (1040, 368))
# Read Template Image
imgSourceBGR = cv2.imread('fandango.jpg', CV_LOAD_IMAGE_COLOR)
...
while True:
    ...
    result = cameraFrameColor
    if (len(matches) > minimumMatchCount):
        srcPts = np.float32([kpSource[m.trainIdx].pt for m in matches]).reshape(-1,1,2)
        dstPts = np.float32([kpCam[m.queryIdx].pt for m in matches]).reshape(-1,1,2)

        M, mask = cv2.findHomography(srcPts, dstPts, cv2.RANSAC, 5.0)
        matchesMask = mask.ravel().tolist() # mask doesn't actually get used, this can probably go
        h,w,d = imgSourceBGR.shape
        pts = np.float32([ [0,0], [0,h-1], [w-1,h-1], [w-1,0] ]).reshape(-1,1,2)
        dst = cv2.perspectiveTransform(pts, M)
        result = cv2.polylines(cameraFrameColor, [np.int32(dst)], True, 255, 3, cv2.LINE_AA)
...
print "Average number of matches: " + str(np.mean(nMatches))
