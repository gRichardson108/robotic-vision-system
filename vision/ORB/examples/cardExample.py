...
cap = cv2.VideoCapture('exampleData/horseman.mp4')
fourcc = cv2.VideoWriter_fourcc(*'MP4V')
out = cv2.VideoWriter('output.mp4', fourcc, 30.0, (1040, 368))
# Read Template Image
imgSourceBGR = cv2.imread('exampleData/Horseman_small.jpg', CV_LOAD_IMAGE_COLOR)
...
while True:
    ...
    if (len(matches) > minimumMatchCount):
        ...
        result[:h1, w2:w1+w2] = cv2.polylines(cameraFrameColor, [np.int32(dst)], True, 255, 3, cv2.LINE_AA)
